...
- **Task priority** — Tasks have an integer `priority` column (1=High, 2=Normal, 3=Low, DB/PGlite default 2). Shown as a color-coded badge in the task table (green=High, grey=Normal, red=Low). Selectable in create/edit task dialog.
- **RAM Guard** — `RamGuard` component mounted in `AppLayout`. Watches `useSystemMetrics` (Electron-only). When RAM % ≥ threshold and task list is hydrated: if auto-stop OFF → shows a modal dialog listing running tasks sorted Low→Normal→High with per-task Stop buttons; if auto-stop ON → silently stops tasks one at a time (3s pause between stops, re-checks threshold after each). High-priority tasks (priority=1) are never auto-stopped. Dialog auto-closes when RAM drops below threshold. 60s debounce applied on alert fire (not per-dismiss). Settings card in Settings page is Electron-only.
- **RAM Guard settings** — Stored in `localStorage` under key `ram-settings` (`{ enabled, threshold, autoStop }`). Utilities exported from `RamGuard.tsx`: `loadRamGuardSettings`, `saveRamGuardSettings`. Settings page dispatches `ram-guard-settings-changed` custom event to sync live.
