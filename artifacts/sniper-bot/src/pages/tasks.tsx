      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-border/20">
        ...
        <div className="flex items-center gap-1">
          <Button
            ...
            onClick={handleCopy}
            data-testid={`button-copy-logs-${taskId}`}
            title="Copy logs"
          >
          ...
          <Button
            ...
            className="h-6 w-6 text-muted-foreground hover:text-foreground"
            onClick={clear}
            data-testid={`button-clear-logs-${taskId}`}
            title="Clear logs"
          >
            <X className="w-3 h-3" />
          </Button>
        ...
        {logs.length === 0 ? (
          ...
          logs.map((entry: TaskLogEntry, i: number) => (
            <div key={i} className="flex gap-2 leading-5">
              ...
              <span
                className={`font-bold w-10 shrink-0 ${LOG_COLORS[entry.level] ?? "text-foreground"}`}
              >
                {entry.level}
              </span>
              <span
                className={`flex-1 break-all ${LOG_COLORS[entry.level] ?? "text-foreground/80"}`}
              >
                {entry.message}
              </span>
            </div>
...
export default function TasksPage() {
  ...
  const activeTasks = tasks.filter(
    (t) =>
      !["idle", "stopped", "failed", "success"].includes(
        getStatus(t),
      ),
  ).length;
        ...
        <div className="flex items-center gap-2">
          ...
          {activeTasks > 0 && (
            <span
              ...
              data-testid="text-active-task-count"
            >
              {activeTasks} running
            </span>
          )}
                      ...
                      <FormItem>
                        ...
                        >
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value="Pokemon Center">
                              Pokemon Center
                            </SelectItem>
                          </SelectContent>
                        </Select>
              ...
              <tr>
                ...
                >
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p>
                      No tasks found. Create your first task to start sniping.
                    </p>
                  </div>
                </td>
              </tr>
                ...
                return (
                  <>
                    ...
                    >
                      ...
                      <td
                        className="px-4 py-3 font-mono text-xs max-w-[200px] truncate text-primary/80"
                        title={task.productUrl ?? task.productKeywords ?? ""}
                      >
                        {task.productUrl || task.productKeywords || "-"}
                      </td>
                      <td className="px-4 py-3 text-muted-foreground">
                        {profiles.find((p) => p.id === task.profileId)?.name ??
                          "Unknown"}
                      </td>
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex items-center gap-1.5 px-2 py-1 rounded text-xs font-mono tracking-wide ${cfg.cls}`}
                          data-testid={`status-task-${task.id}`}
                        >
                          {cfg.dot && (
                            <span
                              className={`w-1.5 h-1.5 rounded-full bg-current ${cfg.dot}`}
                            />
                          )}
                          {cfg.label}
                        </span>
                      ...
                      <td
                        className="px-4 py-3 text-right space-x-1 opacity-0 group-hover:opacity-100 transition-opacity"
